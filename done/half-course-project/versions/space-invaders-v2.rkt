...
(define (generate-invaders l)
  (cond [(> 6 (count l)) (generate-invaders (cons (make-invader (random (+ 5 (- WIDTH (/ (image-width INVADER) 2))))
                                                                (* 2 (image-height INVADER)) ;; when move implemented change to -2
                                                                (if (= (random 2) 1)
                                                                    1
                                                                    -1)) l))]
        [else l]))
...
(define (move-invaders lom) lom)

;; ListOfInvaders -> Boolean
;; produces true if an invader has hit the ground or the tank
;; !!!
(define (invader-arrived? lom) ...)


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;                                    ;;
;;      CONNECTED (missile-hit?)      ;;
;;                                    ;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(define (tock g) (make-game (generate-invaders (game-invaders g))
                            (filter-missiles (move-missiles (game-missiles g)))   ;filter-missiles isn't working
...
(define (render g) (render-invaders (game-invaders g) (render-missiles (filter-missiles (game-missiles g)) (render-tank g))))
